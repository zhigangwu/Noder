...
- (void)configWithItem:(Hasnot_read_messages *)hasnot_read
{
    ...
    self.themelabel.text = [NSString stringWithFormat:@"主题:%@",hasnot_read.topic.title];
    
    self.contentLabel.text = hasnot_read.reply.content;
    //    UILabel 加载HTML
    NSAttributedString *attString = [[NSAttributedString alloc] initWithData:[hasnot_read.reply.content dataUsingEncoding:NSUnicodeStringEncoding]
                                                                     options:@{
