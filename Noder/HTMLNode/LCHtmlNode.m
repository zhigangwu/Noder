...
@property (nonatomic, strong) ASScrollNode *contentView;
...
- (instancetype)init{
    ...
    if (self) {
        ...
        ASScrollNode *contentView = [[ASScrollNode alloc] init];
        self.contentView = contentView;
        [self addSubnode:contentView];
        self.contentView.automaticallyManagesSubnodes = YES;
        self.contentView.automaticallyManagesContentSize = YES;
    }
    return self;
}
...
- (void)setNodes:(NSOrderedSet *)nodes{
    ...
    [self.contentView setLayoutSpecBlock:^ASLayoutSpec*(__kindof ASDisplayNode * _Nonnull node, ASSizeRange constrainedSize) {
        ...
        ASInsetLayoutSpec *insetLayout = [ASInsetLayoutSpec insetLayoutSpecWithInsets:UIEdgeInsetsMake(16, 16, 0, 16) child:layout];
...
- (ASLayoutSpec *)layoutSpecThatFits:(ASSizeRange)constrainedSize {
    return [ASWrapperLayoutSpec wrapperWithLayoutElement:self.contentView];
}
